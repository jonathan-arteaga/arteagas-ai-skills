---
name: apple-swiftui
description: "Build or refactor SwiftUI screens, state, navigation, and presentation in iOS or macOS apps."
license: MIT
metadata:
...
- Read `references/app-wiring.md` when the requested app needs a root navigation/dependency shell; do not add tabs or sheets to a single-view utility without a product need.
- Add only the routes and presentation state the requested flows need, using the skeletons as examples.
...
Preserve the existing architecture and target OS. Apply patterns only to the requested screen or behavior; do not introduce a router, migrate observation, or add test infrastructure as an unrelated improvement. Complete authorized implementation and relevant checks without a new confirmation round. Report unavailable simulator/runtime checks honestly.
