...
- **No glowing CSS orb / sphere / ring.** A generic AI-tool tell. Lumen explicitly refuses it. Pick from the apparatus family or build a new precision object — never a generic ball of light.
- **No `<img>` for the apparatus.** Pure CSS + SVG. Figma-exported SVG paths fail this — the apparatus is constructed in code.
- **No sentence-case or title-case prose.** All headlines, lede, body, buttons, nav, captions, brand wordmarks, footer copy, person names, place names, and legal-entity names are lowercase. Mono labels are the ONLY uppercase surface (eyebrows, callouts, meter labels, stat labels).
- **No `font-style: italic` anywhere.** The verb landmark is built from colour + a 1px underline, not from a glyph variation. Italic body emphasis, italic founder signatures, italic captions — all banned. `<em>` is repurposed as the verb landmark and renders upright.
