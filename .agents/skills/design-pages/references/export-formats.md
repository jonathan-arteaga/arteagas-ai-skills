...
Tailwind v4 reads CSS variables inside `@theme` and generates utilities (`bg-paper`, `text-ink`, etc.) automatically. The translation from design-pages tokens is direct — same variable names, same OKLCH values:
...
@theme {
  ...
  /* Spacing — Tailwind reads --spacing-* by default; we keep design-pages' --space-* names alongside */
  --spacing-3xs: 0.25rem;
  --spacing-2xs: 0.5rem;
  --spacing-xs:  0.75rem;
  --spacing-sm:  1rem;
...
Notes:
- Tailwind v4 expects `--spacing-*` (with `ing`) for the spacing utilities. We mirror design-pages' `--space-*` to `--spacing-*` so both names work.
...
The translation from design-pages tokens:
...
:root {
  ...
  --accent:               58%   0.16  60;     /* accent (same as primary in design-pages) */
...
Then the user's `tailwind.config.ts` (or v4 `@theme`) reads them with `oklch(var(--background))`. design-pages' accent → shadcn's primary is the canonical mapping; secondary → paper-3.

If the user wants a dark variant, mirror with the dark theme tokens under a `.dark` selector (design-pages' Midnight / Bloom / Terminal supply the values).
