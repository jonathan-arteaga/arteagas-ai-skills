...
Both paths produce a `design.md` design-pages can read on subsequent runs; the file format is uniform once written.

## Scope

- **Page-builds only.** Skip on component-scope — a single component is too small to be a system.
- **Multi-page redesign keeps existing behaviour.** `design-pages redesign --multi-page` produces the heavyweight `design.md` per [`verbs/redesign.md`](verbs/redesign.md) § Multi-page flow. That flow already implies a locked system, so the rule there is unchanged.
...
Locked design system. Future design-pages runs read this file first; pages defer
...
design.md with Tailwind exports"* (or the format you want) — design-pages will
...
- All future design-pages runs READ `design.md` first; subsequent picks (genre / theme / typography / motion / CTA voice) defer to it.
