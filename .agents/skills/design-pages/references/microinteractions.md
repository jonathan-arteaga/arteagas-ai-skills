...
- **Restraint, not restraint-as-personality.** design-pages is not "no motion." design-pages is *the right motion in the right place.* A drag handle that springs into focus on grab is good. A page where every card pulses on hover is slop.
...
1. **`transition-all`.** Every property animating, including ones that should be instant (visibility, display, focus rings). Always specify the properties. The class is banned in design-pages output.
