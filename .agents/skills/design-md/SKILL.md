...
Read lifecycle signals while tracing: `@deprecated` annotations, Storybook or documentation status badges, and migration notes. A shared primitive marked deprecated, experimental, or retired is not a governing source even when the product still renders it; when the source names its replacement, the replacement governs.
...
Include a Don't only when a governing source states an explicit prohibition. An explicit deprecation marker counts as one.

When a governing source states a component's unsupported uses, the boundary between two similar components, or a token's usage limit, record it in Components or Do's and Don'ts. That rule cannot be recovered from the code alone, and a reader without it will guess.
