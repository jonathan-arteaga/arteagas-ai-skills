...
- Without a governing outline token, prefer the neutral fallback over an arbitrary palette value such as slate-900, zinc-900, `#0a0a0a`, `#111827`, or `#f5f5f7`. Tinted outlines pick up the surrounding surface color and read as dirt on the image edge.
- Preserve a governing outline token; otherwise avoid choosing accent or ink solely because it is available. The outline is a neutral separator, not a themed element.
