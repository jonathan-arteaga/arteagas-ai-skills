...
Read this reference when writing or migrating tests.

## Testing — Swift Testing by default

Preserve the project’s test framework. For a new test target, prefer **Swift Testing** when supported. XCTest remains required for exactly three things: UI automation (`XCUIApplication`), performance metrics (`XCTMetric`), and tests that must be written in Objective-C or that catch Objective-C exceptions.
